...
class Clix:
    def __init__(self, parent, XML):
        ...
        self.picture = None
        p = self.xmlDial.get("pic")
        if p is not None:
            self.picture = os.path.join(sys.path[0], 'assets', 'figures', 'ha022.jpg')
    ...
    def drawTexture(self, im, draw):
        if self.picture:
            p = Image.open( self.picture ).convert('RGB').convert('P', palette=Image.ADAPTIVE)
            im.paste( p,  ( 0,  0 ) )

        if self.drawWindow:
            draw.polygon( [(35, 65), (35, 135),  (92, 135), (64, 65)], fill="white", outline="black")
        
        if ( self.currentClick == 1 ):
            draw.line( ( 37,  67,  37,  133 ),  fill="green", width=2 )
        else:
            line = self.getLine()
            if line:
                draw.line( ( 37,  67,  37,  133 ),  fill=line, width=2 )
        ...
        draw.text( (4,  136),  self.getName(),  font=dialTopFont,  fill="black" )
        draw.text( (4,  4),  self.getId(),  font=clickNumFont,  fill="black" )

        draw.text( (150-25-(3*len(str(self.getPoints()))), 33),  self.getPoints(),  font=clickNumFont,  fill="black")
            
        r = self.getRange()
        if r:
            draw.text( (102, 68),  r[0],  font=font,  fill="black" )
        ...
        if click:
            self.drawValues( im, draw,  click,  font, -20 )
        else:
            offset = 0
            spd_xy = (x-4, y)
            atk_xy = (x-4,  y+dY)
        ...
        if self.spd_symbol_image:
            im.paste( self.spd_symbol_image,  ( 11,  65 ) )
        if self.atk_symbol_image:
            im.paste( self.atk_symbol_image,  ( 11,  90 ) )
        if self.def_symbol_image:
            im.paste( self.def_symbol_image,  ( 11,  115 ) )
        if self.dmg_symbol_image:
            im.paste( self.dmg_symbol_image,  ( 99,  106 ) )
    ...
    def drawDial(self):
        ...
        if click:
            self.drawValues( im, draw,  click,  font, 0 )
        else:
            offset = 0
            spd_xy = (x-4, y)
            atk_xy = (x-4,  y+dY)
